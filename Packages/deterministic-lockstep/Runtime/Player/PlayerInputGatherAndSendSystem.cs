...
namespace DeterministicLockstep
{
    ...
    public partial class PlayerInputGatherAndSendSystem : SystemBase
    {
        ...
        {
            foreach (var (inputDataToSend, connectionReference, tickRateInfo, connectionEntity) in SystemAPI
                         .Query<RefRW<PlayerInputDataToSend>, RefRO<NetworkConnectionReference>, RefRW<TickRateInfo>>()
                         .WithAll<PlayerSpawned, GhostOwnerIsLocal>().WithEntityAccess())
            {
                ...
                {
                    PlayerInput = new Vector2(inputDataToSend.ValueRO.horizontalInput,
                        inputDataToSend.ValueRO.verticalInput),
