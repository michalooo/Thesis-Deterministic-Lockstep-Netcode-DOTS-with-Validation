...
public partial class CalculateTickSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        foreach (var (tickRateInfo, storedTicksAhead, connectionEntity) in SystemAPI.Query<RefRW<TickRateInfo>, RefRW<StoredTicksAhead>>().WithAll<GhostOwnerIsLocal, PlayerSpawned>().WithEntityAccess())
        {
            ...
            if (tickRateInfo.ValueRO.delayTime <= 0) // We are ready to try to send the next tick
            {
                ...
                else if(storedTicksAhead.ValueRO.entries.Count > 0) // otherwise we can try to proceed with simulation steps (different than presentation steps)
                {
                    ...
                    while(storedTicksAhead.ValueRO.entries.Count > tickRateInfo.ValueRO.tickAheadValue) // We are trying to catch up with the server
                    {
                        ...
                        UpdateComponentsData(storedTicksAhead.ValueRW.entries.Dequeue()); // it will remove it so no reason for dispose method for arrays?
                        
                        EntityManager.SetComponentEnabled<PlayerInputDataToSend>(connectionEntity, true);
                        EntityManager.SetComponentEnabled<PlayerInputDataToUse>(connectionEntity, true);

                        World.PushTime(new TimeData(elapsedTime: 0f, deltaTime: 1f / tickRateInfo.ValueRO.tickRate));
    ...
    /// <summary>
    /// Function responsible for updating the player components based on the given RPC.
    /// </summary>
